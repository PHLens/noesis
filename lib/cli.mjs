...
import { CompressionError, printCompressionSummaryUsage, printCompressionUsage, runCompressionCommand } from './compression.mjs';
import { EvalHandoffError, printEvalHandoffUsage, printEvalUsage, runEvalCommand } from './eval-handoff.mjs';
import { EvalReplayError, printEvalReplayUsage } from './eval-replay.mjs';
import { EventError, printEventCheckUsage, printEventPromoteUsage, printEventUsage, runEventCommand } from './event.mjs';
import { LaunchError, printLaunchUsage, printListUsage, printRemoveUsage, runLaunchCommand, runListCommand, runRemoveCommand } from './launch.mjs';
import { OwnerHandoffError, printOwnerHandoffUsage, printOwnerOutcomeUsage, printOwnerUsage, runOwnerCommand } from './owner-handoff.mjs';
import { RouteError, printRouteUsage, runRouteCommand } from './route.mjs';
import { SetupError, printSetupUsage, runSetupCommand } from './setup.mjs';
...
import { UpdateError, printUpdateUsage, runUpdateCommand } from './update.mjs';


const TOP_USAGE = `Usage: noesis <command> [args]

Commands:
  launch                        Create/resume a task instance and start or bind a runtime.
  update                        Update Noesis-managed local tooling and components.
  list                          List task instances, compatibility agent homes, and Slock workspaces.
  remove                        Remove Noesis/pamem launch integration from a workspace.
  init                          Create Noesis-owned bootstrap state.
  setup                         Advanced workspace prepare/bootstrap.
  ...
  owner                         Handoff approved proposals to owner lanes.
  eval                          Handoff approved eval proposals and run golden replays.
  compression                   Summarize repeated and stale learning artifacts.
...
Examples:
  noesis launch --name research-routing --role researcher --runtime codex
  noesis launch --role reviewer --runtime codex --rm
  noesis update
  noesis list
  noesis init --workspace /path/to/workspace --with pamem,loreforge
  noesis launch --profile researcher --runtime slock --workspace /path/to/slock-workspace
  ...
  noesis owner handoff .noesis/proposals/example.json
  noesis owner outcome .noesis/proposals/example.json --status owner_pending --ref pr:https://github.com/org/repo/pull/1
  noesis eval handoff .noesis/proposals/example.json
  noesis eval replay --json
...
export function main(argv = process.argv.slice(2)) {
  try {
    ...
    if (command === 'launch') {
      if (hasHelpFlagBeforeTerminator(rest)) {
        printLaunchUsage();
        return 0;
      }
      return runLaunchCommand(rest, { version: packageVersion() });
    }
    if (command === 'update') {
      if (hasHelpFlag(rest)) {
        printUpdateUsage();
      ...
      return runUpdateCommand(rest, { version: packageVersion() });
    }
    if (command === 'list') {
      if (hasHelpFlag(rest)) {
        printListUsage();
    ...
    if (command === 'owner') {
      return runOwnerCommand(rest);
    }
    if (command === 'eval') {
      return runEvalCommand(rest);
    }
    if (command === 'compression') {
      return runCompressionCommand(rest);
    }
    ...
    if (error instanceof UsageError || error instanceof SkillManagerError || error instanceof BootstrapError || error instanceof SetupError || error instanceof LaunchError || error instanceof UpdateError || error instanceof PromoteError || error instanceof ProposalError || error instanceof EventError || error instanceof RouteError || error instanceof OwnerHandoffError || error instanceof EvalHandoffError || error instanceof EvalReplayError || error instanceof CompressionError) {
...
function printHelp(topics) {
  ...
  if (topics[0] === 'owner') {
    return printOwnerHelp(topics.slice(1));
  }
  if (topics[0] === 'eval') {
    return printEvalHelp(topics.slice(1));
  }
  if (topics[0] === 'compression') {
    return printCompressionHelp(topics.slice(1));
  }
  if (topics[0] === 'launch') {
    printLaunchUsage();
    return 0;
...
Read-only check of the Noesis manifest and umbrella HS readiness.

The JSON report includes grouped readiness sections for Noesis, entry skills,
pamem, LoreForge, and the Noesis skill manager while preserving the detailed
flat checks list.
...
function printOwnerHelp(topics) {
  if (topics.length === 0) {
    printOwnerUsage();
    return 0;
  }
  const [topic, ...extra] = topics;
  if (extra.length > 0) {
    throw new UsageError(`unknown help topic: owner ${topics.join(' ')}`);
  }
  if (topic === 'handoff') {
  ...
  throw new UsageError(`unknown owner command: ${topic}`);
}
...
function printCompressionHelp(topics) {
  if (topics.length === 0) {
  ...
  const [topic, ...extra] = topics;
  if (extra.length > 0) {
    throw new UsageError(`unknown help topic: compression ${topics.join(' ')}`);
  }
  if (topic === 'summary') {
    printCompressionSummaryUsage();
    return 0;
  }
  throw new UsageError(`unknown compression command: ${topic}`);
}
...
function hasHelpFlagBeforeTerminator(tokens) {
  const terminator = tokens.indexOf('--');
  const visible = terminator === -1 ? tokens : tokens.slice(0, terminator);
  return hasHelpFlag(visible);
}
