...
import { EventError, printEventCheckUsage, printEventUsage, runEventCommand } from './event.mjs';
...
export function main(argv = process.argv.slice(2)) {
  try {
    ...
    if (error instanceof UsageError || error instanceof SkillManagerError || error instanceof BootstrapError || error instanceof PromoteError || error instanceof ProposalError || error instanceof EventError) {
...
function printProposalHelp(topics) {
  ...
  if (['list', 'show', 'update'].includes(topic)) {
