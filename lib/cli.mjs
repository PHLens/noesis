...
Target options:
  ...
  --agent <name>                Resolve a Claude agent memory workspace by agent name.
...
Examples:
  ...
  noesis skill list --agent developer
...
function parseSkillArgs(argv) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    } else if (token === '--agent' || token === '--agent-name') {
      args.agentName = requireValue(tokens, ++index, token);
    } else if (token.startsWith('--agent=')) {
      args.agentName = token.slice('--agent='.length);
    } else if (token.startsWith('--agent-name=')) {
      args.agentName = token.slice('--agent-name='.length);
