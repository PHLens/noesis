...
import { printEvalReplayUsage, runEvalReplayCommand } from './eval-replay.mjs';
...
export function runEvalCommand(tokens) {
  ...
  if (command === 'help') {
    ...
    if (rest.length === 1 && rest[0] === 'replay') {
  ...
  if (command === 'replay') {
    return runEvalReplayCommand(rest);
  }
  if (command !== 'handoff') {
...
Commands:
  ...
  replay [case-file...]          Replay route/proposal golden cases.
