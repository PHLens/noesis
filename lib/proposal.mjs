...
const RESERVED_STATUSES = new Set(['applied', 'owner_pending', 'materialized', 'merged', 'failed']);
const OUTCOME_STATUSES = new Set(['not_applied', 'owner_pending', 'materialized', 'merged', 'rejected', 'failed']);
...
function buildSummaryReport(proposalDir, records, args) {
  ...
  const reviewQueue = buildReviewQueue(warnings);
  ...
  const summary = {
    ...
    owner_outcome_pending_count: warnings.filter((warning) => warning.code === 'owner_outcome_pending').length,
    review_queue_count: reviewQueue.length,
    materialized_count: validRecords.filter((record) => record.proposal.outcome && record.proposal.outcome.status === 'materialized').length,
    merged_count: validRecords.filter((record) => record.proposal.outcome && record.proposal.outcome.status === 'merged').length,
    failed_count: validRecords.filter((record) => record.proposal.outcome && record.proposal.outcome.status === 'failed').length,
...
function buildQueueWarnings(records, { now, staleDays }) {
  ...
  for (const record of records) {
    ...
    if (!isPlainObject(proposal.outcome) || !OUTCOME_STATUSES.has(proposal.outcome.status)) {
      warnings.push(queueWarning('warning', 'unsupported_outcome_status', record, `Proposal outcome.status is missing or unsupported: ${proposal.outcome && proposal.outcome.status ? proposal.outcome.status : 'missing'}`));
    }
    ...
    if (record.status === 'approved' && isPlainObject(proposal.outcome) && ['owner_pending', 'materialized'].includes(proposal.outcome.status)) {
      warnings.push(queueWarning('warning', 'owner_outcome_pending', record, `Owner outcome is ${proposal.outcome.status}; final merged/rejected/failed outcome is still pending.`));
    }
  }
  return warnings;
}
...
function buildReviewQueue(warnings) {
  return warnings
    .map((warning) => reviewQueueItem(warning))
    .filter(Boolean)
    .sort(compareReviewQueueItems);
}


function reviewQueueItem(warning) {
  const selector = warning.proposal_id || warning.path;
  if (!selector) return null;
  const base = {
    severity: warning.severity,
    code: warning.code,
    proposal_id: warning.proposal_id || null,
    status: warning.status || null,
    target_owner: warning.target_owner || null,
    path: warning.path,
    reason: warning.message,
  };

  if (warning.code === 'invalid_artifact') {
    return {
      ...base,
      priority: 'p0',
      action: 'repair_artifact',
      next_step: 'Repair or remove the invalid proposal JSON artifact before reviewing the queue.',
      suggested_command: `noesis proposal show ${shellQuote(selector)} --json`,
    };
  }
  if (warning.code === 'unsupported_schema_version' || warning.code === 'unsupported_status') {
    return {
      ...base,
      priority: 'p1',
      action: 'repair_artifact',
      next_step: 'Normalize the proposal artifact before owner handoff.',
      suggested_command: `noesis proposal show ${shellQuote(selector)} --json`,
    };
  }
  if (warning.code === 'high_risk_pending_review') {
    return {
      ...base,
      priority: 'p1',
      action: 'review_proposal',
      next_step: 'Review the high-risk proposal and approve, reject, or supersede it.',
      suggested_command: `noesis proposal show ${shellQuote(selector)} --json`,
    };
  }
  if (warning.code === 'stale_pending_review') {
    return {
      ...base,
      priority: 'p2',
      action: 'review_proposal',
      next_step: 'Review the stale proposal and approve, reject, or supersede it.',
      suggested_command: `noesis proposal show ${shellQuote(selector)} --json`,
    };
  }
  if (warning.code === 'approved_owner_handoff_pending') {
    return {
      ...base,
      priority: 'p1',
      action: 'handoff_owner',
      next_step: 'Create a Noesis-owned owner handoff artifact for the approved proposal.',
      suggested_command: `noesis owner handoff ${shellQuote(selector)} --json`,
    };
  }
  if (warning.code === 'owner_outcome_pending') {
    return {
      ...base,
      priority: 'p1',
      action: 'record_owner_outcome',
      next_step: 'Record the owner-side terminal outcome or latest materialization ref on the proposal.',
      suggested_command: `noesis owner outcome ${shellQuote(selector)} --status merged --ref commit:<sha> --json`,
    };
  }
  return null;
}
...
function compareReviewQueueItems(left, right) {
  ...
  const leftPriority = priorities[left.priority] ?? 99;
  const rightPriority = priorities[right.priority] ?? 99;
  if (leftPriority !== rightPriority) return leftPriority - rightPriority;
  const leftId = left.proposal_id || left.path || '';
  const rightId = right.proposal_id || right.path || '';
...
function validateRequestedStatus(status) {
  if (RESERVED_STATUSES.has(status)) {
    throw new ProposalError(`${status} is reserved for owner outcome flow and cannot be set by proposal review`);
  }
...
function printSummaryHuman(report) {
  ...
  if (report.review_queue.length > 0) {
    ...
    for (const item of report.review_queue) {
      const id = item.proposal_id || item.path;
      console.log(`- ${item.priority} ${item.action} ${id}: ${item.next_step}`);
      console.log(`  command: ${item.suggested_command}`);
    }
  }
  console.log('Downstream execution: not run');
}
...
Commands:
  ...
  summary                       Summarize proposal queue status, warnings, and review next steps.
...
Summarize proposal queue status, owner distribution, warning conditions, and
...
and owner-handoff conditions with suggested next commands, without applying
owner changes.`);
    return;
  }
  if (command === 'show') {
