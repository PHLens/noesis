...
function buildQueueWarnings(records, { now, staleDays }) {
  ...
  for (const record of records) {
    ...
    if (record.status === 'approved' && isPlainObject(proposal.outcome) && proposal.outcome.status === 'owner_pending') {
      warnings.push(queueWarning('warning', 'owner_outcome_pending', record, 'Owner handoff has been recorded but final owner outcome is still pending.'));
    }
  }
  return warnings;
}
