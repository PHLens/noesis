...
export function createPromoteCheckReport(args) {
  return loadAndCheckRequest(args.requestPath, args.workspace);
}


export function createPromotePlanReport(args) {
  ...
  if (checkReport.summary.error_count > 0) {
    ...
    return { request, proposals: [], report };
  }
...
function runPromoteCheck(args) {
  const { report } = createPromoteCheckReport(args);
...
function runPromotePlan(args) {
  const { report } = createPromotePlanReport(args);
  if (args.json) printJson(report);
