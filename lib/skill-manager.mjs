...
const PROTECTED_PAMEM_SKILLS = new Set(['memory-lint', 'memory-rule']);
...
function resolveAgentRoot(agentId) {
  const result = spawnPamemStatus(['status', '--agent-id', agentId, '--json'], {
    encoding: 'utf8',
  });
  if (result.error) {
...
function spawnPamemStatus(args, options) {
  const candidates = pamemCommandCandidates();

  let firstResult = null;
  for (const command of candidates) {
    const result = spawnSync(command, args, options);
    if (isPamemMissingConfigForAgentId(result)) {
      firstResult ||= result;
      continue;
    }
    return result;
  }
  return firstResult || spawnSync(candidates[candidates.length - 1], args, options);
}


function pamemCommandCandidates() {
  const override = configuredPamemBin();
  if (override) return [override];
  const candidates = [];
  if (fs.existsSync(PAMEM_BIN)) candidates.push(PAMEM_BIN);
  candidates.push('pamem');
  return candidates;
}


function configuredPamemBin() {
  const value = process.env.NOESIS_PAMEM_BIN;
  if (typeof value !== 'string' || value.trim() === '') return '';
  return expandPath(value.trim());
}


function isPamemMissingConfigForAgentId(result) {
  if (!result || result.status === 0 || result.error) return false;
  const detail = `${result.stderr || ''}${result.stdout || ''}`;
  return detail.includes('pamem config not found for root:') && detail.includes('/pamem/agents/');
}
...
function verifySkills(target, name = null, sourceArg = null) {
  ...
  const failures = listed.skills
    .filter((skill) => skill.status !== 'ok' && !pamemProvidedRuntimeSkill(skill))
...
function pamemProvidedRuntimeSkill(skill) {
  if (!PROTECTED_PAMEM_SKILLS.has(skill.name)) return false;
  return ['ok', 'broken'].includes(skill.runtimes?.codex?.status)
    && skill.runtimes?.codex?.kind === 'symlink'
    && skill.runtimes?.claude?.status === 'missing';
}
...
function setClaudePlugin(target, name, pluginKey, enabled) {
  ...
  if (current.status === 'invalid' && !(enabled && current.enabled && current.recoverable)) {
  ...
  if (enabled) {
    if (current.enabled) {
      if (current.status === 'invalid' && current.recoverable) {
        const cliAction = runClaudePluginCli(target, pluginKey, true, 'update');
        return {
          type: 'claude-plugin',
          capability: name,
          key: pluginKey,
          path: settingsPath,
          action: 'refreshed',
          reason: current.error_kind || 'recoverable-invalid',
          ...cliAction,
        };
      }
      return {
...
function runClaudePluginCli(target, pluginKey, enabled, subcommandOverride = null) {
  const scope = claudePluginScope(target);
  const subcommand = subcommandOverride || (enabled ? 'install' : 'uninstall');
...
function runPamemBootstrap(target, command) {
  ...
  const pamemCommand = configuredPamemBin() || (fs.existsSync(PAMEM_BIN) ? PAMEM_BIN : 'pamem');
...
function claudePluginInfo(target, pluginKey) {
  ...
  if (info.enabled) {
    return claudePluginCliInfo(target, pluginKey, {
      ...info,
      status: 'ok',
...
function claudePluginCliInfo(target, pluginKey, baseInfo) {
  ...
  const record = {
    ...baseInfo,
    status: found.enabled === true ? 'ok' : 'missing',
    ...
    project_path: typeof found.projectPath === 'string' ? found.projectPath : null,
  };
  if (!record.enabled) return record;

  const pluginErrors = claudePluginErrors(found);
  if (pluginErrors.length > 0) {
    return {
      ...record,
      status: 'invalid',
      recoverable: true,
      error_kind: 'plugin-errors',
      errors: pluginErrors,
      error: `Claude plugin ${pluginKey} reported errors: ${pluginErrors.join('; ')}`,
    };
  }
  const standardHook = pamemClaudeStandardHookInfo(pluginKey, record.install_path);
  if (standardHook) {
    record.standard_hook = standardHook;
    if (standardHook.status !== 'ok') {
      return {
        ...record,
        status: 'invalid',
        recoverable: true,
        error_kind: 'standard-hook',
        error: standardHook.error,
      };
    }
  }
  return record;
...
function claudePluginErrors(plugin) {
  if (!Array.isArray(plugin.errors)) return [];
  return plugin.errors.filter((item) => typeof item === 'string' && item.trim() !== '');
}


function pamemClaudeStandardHookInfo(pluginKey, installPath) {
  if (pluginKey !== RUNTIME_CAPABILITIES.pamem.claude_key) return null;
  const hookPath = typeof installPath === 'string' && installPath.trim() !== ''
  ...
  if (!hookPath) {
    return {
      ...info,
      error: `Claude plugin ${pluginKey} did not report an installPath for hook verification`,
    };
  }
  if (!isFile(hookPath)) {
    return {
      ...info,
      error: `Claude plugin ${pluginKey} is missing standard hook file: ${hookPath}`,
    };
  }
  const parsed = readJsonObject(hookPath);
  if (parsed.status !== 'ok') {
    return {
      ...info,
      status: 'invalid',
      error: `Claude plugin ${pluginKey} hook file is invalid: ${parsed.error}`,
    };
  }
  const hooks = parsed.value.hooks?.SessionStart;
  const hook = Array.isArray(hooks)
    ? hooks.flatMap((entry) => (Array.isArray(entry?.hooks) ? entry.hooks : []))
      .find((item) => item?.type === 'command' && typeof item.command === 'string' && item.command.includes('memory-session-start.sh'))
    : null;
  if (!hook) {
    return {
      ...info,
      status: 'missing',
      error: `Claude plugin ${pluginKey} standard hook file does not expose memory-session-start.sh`,
    };
  }
  return {
    ...info,
...
function codexConfigInfo(configPath) {
  ...
  info.codex_hooks = /^[ \t]*(?:codex_)?hooks[ \t]*=[ \t]*true[ \t]*$/m.test(text);
...
function classifySource(sourcePath) {
  ...
  return { kind: 'local', root: path.dirname(resolvedSource) };
}
