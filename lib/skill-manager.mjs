...
function resolveTarget(args) {
  const selected = [Boolean(args.workspace), Boolean(args.agentId), Boolean(args.agentName), Boolean(args.globalScope)].filter(Boolean).length;
  if (selected > 1) {
    throw new SkillManagerError('--workspace, --agent-id, --agent, and --global are mutually exclusive');
  }
  ...
  if (args.agentName) {
    const root = resolveClaudeAgentMemoryRoot(args.agentName);
    requireDirectory(root, `memory root for agent ${args.agentName}`);
    return {
      scope: 'workspace',
      kind: 'agent-home',
      root,
      resolver: 'claude-agent',
...
function resolveClaudeAgentMemoryRoot(agentName) {
  requireLinkName(agentName, 'agent name');
  const agentPath = path.join(userHome(), '.claude', 'agents', `${agentName}.md`);
  if (!isFile(agentPath)) {
    throw new SkillManagerError(`Claude agent definition not found: ${agentPath}`);
  }
  const text = fs.readFileSync(agentPath, 'utf8');
  const frontmatter = parseFrontmatter(text, agentPath);
  const memory = frontmatter.memory;
  if (!memory) {
    throw new SkillManagerError(`Claude agent definition has no memory field: ${agentPath}`);
  }
  if (memory === 'user') {
    return path.join(userHome(), '.claude', 'agent-memory', agentName);
  }
  return expandPath(memory);
}
...
function runPamemBootstrap(target, command) {
  ...
  if (command === 'install' && target.kind === 'agent-home') args.push('--agent-home');
...
function parseFrontmatter(text, file) {
  ...
  if (end < 0) throw new SkillManagerError(`unterminated frontmatter in ${file}`);
  const values = {};
  for (const line of lines.slice(1, end)) {
    const match = /^([A-Za-z0-9_-]+):[ \t]*(.*)$/.exec(line);
    if (!match) continue;
    const [, key, rawValue] = match;
    values[key] = stripYamlString(rawValue.trim());
  }
  return values;
}
