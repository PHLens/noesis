...
const DEFAULT_WITH = ['pamem', 'loreforge'];
...
function runDoctor(args) {
  ...
  if (manifest) {
    ...
    checkNoesisEntrySkill(workspace, manifest, checks);
    checkComponents(workspace, manifest, checks);
  }

  const report = buildDoctorReport(workspace, configPath, checks, manifest);
...
export function buildManifest({ workspace, enabled, version }) {
  return {
    noesis: {
    ...
    components: {
      ...
      loreforge: {
        enabled: enabled.includes('loreforge') && commandExists('loreforge'),
        owner: 'LoreForge',
...
function checkNoesisEntrySkill(workspace, manifest, checks) {
  const skill = manifest.noesis?.entry_skill;
  if (!skill) {
    checks.push(check('noesis.entry_skill', 'warning', false, 'Noesis entry skill is not declared'));
    return;
  }
  const visible = entrySkillVisible(workspace, skill);
  checks.push(check(
    'noesis.entry_skill',
    visible ? 'info' : 'warning',
    visible,
    visible ? `${skill} entry skill is visible` : `${skill} entry skill is not visible`,
    { skill },
  ));
}
...
function checkComponents(workspace, manifest, checks) {
  for (const [name, component] of Object.entries(manifest.components || {})) {
    ...
    if (component.required_entry_skill_source) {
      const sourcePath = path.resolve(workspace, component.required_entry_skill_source);
      const skillMd = path.join(sourcePath, 'SKILL.md');
      checks.push(check(
        `component.${name}.entry_skill_source`,
        fs.existsSync(skillMd) ? 'info' : 'warning',
        fs.existsSync(skillMd),
        fs.existsSync(skillMd) ? `${name} entry skill source exists` : `${name} entry skill source is missing SKILL.md`,
        { path: sourcePath, skill_md: skillMd },
      ));
    }
...
function commandExists(command) {
  ...
  if (typeof command === 'string' && command.includes(path.sep)) {
    return fs.existsSync(command.split(/\s+/)[0]);
  }
...
function buildDoctorReport(workspace, configPath, checks, manifest = null) {
  const summary = {
  ...
  const status = summary.error_count > 0 ? 'failed' : summary.warning_count > 0 ? 'warning' : 'ok';
  return {
    status,
    workspace,
    config_path: configPath,
    summary,
    readiness: buildReadiness(status, checks, manifest),
    checks,
  };
}


function buildReadiness(status, checks, manifest) {
  const sections = {
    noesis: readinessSection('noesis', 'Noesis control plane', checks.filter((item) => (
      item.id.startsWith('manifest.')
      || item.id.startsWith('paths.')
    ))),
    entry_skills: readinessSection('entry_skills', 'Entry skills', checks.filter((item) => (
      item.id === 'noesis.entry_skill'
      || item.id.endsWith('.entry_skill')
      || item.id.endsWith('.entry_skill_source')
    ))),
    pamem: componentReadinessSection('pamem', manifest, checks),
    loreforge: componentReadinessSection('loreforge', manifest, checks),
    skill_manager: componentReadinessSection('skill_manager', manifest, checks),
  };
  return {
    status,
    sections,
...
function componentReadinessSection(name, manifest, checks) {
  const componentChecks = checks.filter((item) => item.id.startsWith(`component.${name}.`));
  const component = manifest?.components?.[name];
  if (component && component.enabled === false) {
    return readinessSection(name, componentReadinessLabel(name), componentChecks, { enabled: false, status: 'disabled' });
  }
  return readinessSection(name, componentReadinessLabel(name), componentChecks, { enabled: component?.enabled !== false });
}


function componentReadinessLabel(name) {
  if (name === 'pamem') return 'pamem memory owner component';
  if (name === 'loreforge') return 'LoreForge knowledge component';
  if (name === 'skill_manager') return 'Noesis skill manager';
  return name;
}


function readinessSection(id, label, checks, overrides = {}) {
  const summary = {
    error_count: checks.filter((item) => item.severity === 'error' && item.status !== 'ok').length,
    warning_count: checks.filter((item) => item.severity === 'warning' && item.status !== 'ok').length,
    ok_count: checks.filter((item) => item.status === 'ok').length,
    check_count: checks.length,
  };
  const status = overrides.status
    || (summary.error_count > 0 ? 'failed' : summary.warning_count > 0 ? 'warning' : 'ok');
  return {
    id,
    label,
    status,
...
function doctorHuman(report) {
  ...
  if (report.readiness?.sections) {
    console.log('Readiness:');
    for (const section of Object.values(report.readiness.sections)) {
      console.log(`- ${section.status} ${section.id}: ${section.label}`);
    }
  }
