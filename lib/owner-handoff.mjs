...
const TERMINAL_OUTCOME_STATUSES = new Set(['merged', 'rejected', 'failed']);
const OUTCOME_TRANSITIONS = {
  not_applied: new Set(['owner_pending', 'materialized', 'merged', 'rejected', 'failed']),
  owner_pending: new Set(['materialized', 'merged', 'rejected', 'failed']),
  materialized: new Set(['merged', 'rejected', 'failed']),
  merged: new Set([]),
  rejected: new Set([]),
  failed: new Set([]),
};
...
function runOwnerOutcome(args) {
  ...
  const previousOutcomeStatus = previousOutcome && previousOutcome.status ? previousOutcome.status : 'missing';
  validateOutcomeTransition(previousOutcomeStatus, args.status, record.path);
  const now = new Date().toISOString();
  const newRefs = [
    { kind: 'handoff', ref: handoffPath },
    ...args.refs,
  ];
  const refs = mergeOutcomeRefs(
    previousOutcome && Array.isArray(previousOutcome.refs) ? previousOutcome.refs : [],
    newRefs,
  );
  const outcome = {
    status: args.status,
...
function validateProposalForOutcome(proposal, proposalPath) {
  ...
  const currentStatus = isPlainObject(proposal.outcome) && typeof proposal.outcome.status === 'string'
    ? proposal.outcome.status
    : 'missing';
  if (!OUTCOME_TRANSITIONS[currentStatus]) {
    throw new OwnerHandoffError(`owner outcome requires a supported current outcome.status in ${proposalPath}: ${currentStatus}`);
  }
  if (TERMINAL_OUTCOME_STATUSES.has(currentStatus)) {
    throw new OwnerHandoffError(`owner outcome is already terminal in ${proposalPath}: ${currentStatus}`);
  }
}


function validateOutcomeTransition(previousStatus, nextStatus, proposalPath) {
  const allowed = OUTCOME_TRANSITIONS[previousStatus];
  if (!allowed) {
    throw new OwnerHandoffError(`owner outcome requires a supported current outcome.status in ${proposalPath}: ${previousStatus}`);
  }
  if (!allowed.has(nextStatus)) {
    throw new OwnerHandoffError(`invalid owner outcome transition in ${proposalPath}: ${previousStatus} -> ${nextStatus}`);
  }
}


function mergeOutcomeRefs(existingRefs, newRefs) {
  const refs = [];
  const seen = new Set();
  for (const ref of [...existingRefs, ...newRefs]) {
    if (!isPlainObject(ref) || typeof ref.kind !== 'string' || typeof ref.ref !== 'string') continue;
...
The outcome command writes only the proposal artifact's outcome record and can
append progress from pending/materialized owner work to a terminal merged,
rejected, or failed outcome. It does not call owner commands, apply
memory/wiki/skill/eval changes, or create owner PRs, drafts, commits, or
reports.`);
}
