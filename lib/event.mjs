...
export function createEventCheckReport(args) {
  return loadAndCheckEvent(args.eventPath, args.workspace);
}


export function createEventPromoteReport(args) {
  ...
  if (eventReport.summary.error_count > 0) {
    ...
    return { event, request: null, report };
  }
  ...
  return { event, request, report };
}


function runEventCheck(args) {
  const { report } = createEventCheckReport(args);
...
function runEventPromote(args) {
  const { report } = createEventPromoteReport(args);
  if (args.json) printJson(report);
